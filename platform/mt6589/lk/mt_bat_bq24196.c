 ...
 *  BATTERY VOLTAGE
 ****************************************************************************/
/*begin lenovo-sw xuwei9 2013-3-14,increase lowvol VOL for power on system stability*/
#define BATTERY_LOWVOL_THRESOLD             3550 /* 3450*/
/*end lenovo-sw xuwei9 2013-3-14,increase lowvol VOL for power on system stability*/
...
****************************************************************************/
int g_chr_event = 0;
/*begin lenovo-sw xuwei9 2013-3-14,battery charging icon not update */
/*int bat_volt_cp_flag = 0;*/
/*end lenovo-sw xuwei9 2013-3-14,battery charging icon not update */
...
{
    upmu_set_rg_vcdt_hv_vth(0xA);    //VCDT_HV_VTH, 8.5V->6.5V
}
...
static int set_current_as_temp_bq24196(int temp_CC_value)
{
        ...
        const int STEP_TEMP = 1 ;
        int cc_reg_value = temp_CC_value;
        int tend = 0;
        ...
        //0-10 & 45-50 current limit to 756ma , 10-45 current is normal
        if(((temp>=0)&&(temp<(10+tend*STEP_TEMP)))||((temp>=(45+tend*STEP_TEMP))&&(temp<50)))
        {
            if(cc_reg_value > 756 )
                cc_reg_value = 756; //756MA
        }

        printf("[BATTERY:bq24196] bq24196_set_ac_current:%d \r\n",cc_reg_value);
        return (cc_reg_value-500)/64;
}
/*end,lenovo-sw xuwei9 add 2013-3-7, modify current by temperature*/
...
#else
/*begin,lenovo-sw xuwei9 add 2013-3-7, modify current by temperature*/
        ichg_val = set_current_as_temp_bq24196(g_temp_CC_value);
/*end,lenovo-sw xuwei9 add 2013-3-7, modify current by temperature*/
#endif    
...
/*Begin, lenovo-sw xuwei9 modify 2013-3-13 shorten charge time*/
	bq24196_set_iterm(0x1); //Termination current 128mA->256
/*End, lenovo-sw xuwei9 modify 2013-3-13 shorten charge time*/
#if defined(MTK_JEITA_STANDARD_SUPPORT)        
    if(g_temp_status == TEMP_NEG_10_TO_POS_0)
    {    
...
/*begin lenovo-sw xuwei9 2013-3-26,keep overvol check before temperature check*/
    if( upmu_is_chr_det() == KAL_TRUE)
    {
...
/*end lenovo-sw xuwei9 2013-3-26,keep overvol check before temperature check*/
#if defined(MTK_JEITA_STANDARD_SUPPORT)
    if (Enable_BATDRV_LOG == 1) {
...
void bmt_charger_ov_check(void)
{
     ...
     {
/*begin lenovo-sw xuwei9 2013-3-26,ov=7v then plug in charger ,system path not closed*/
         BMT_status.charger_protect_status = charger_OVER_VOL;
/*end lenovo-sw xuwei9 2013-3-26,ov=7v then plug in charger ,system path not closed*/
         pchr_turn_off_charging_bq24196();
...
void batdrv_init(void)
{
    ...
    BMT_status.TOPOFF_charging_time = 0;
    BMT_status.POSTFULL_charging_time = 0;
/*begin lenovo-sw xuwei9 2013-3-26,50-60 temperature ,system power path keep on*/
    BMT_status.charger_protect_status = 0;
/*end lenovo-sw xuwei9 2013-3-26,50-60 temperature ,system power path keep on*/
    BMT_status.bat_charging_state = CHR_PRE;
