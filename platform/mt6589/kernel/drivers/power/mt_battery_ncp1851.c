...
//liulc1 add
    if((BMT_status.temperature >= 45)||((BMT_status.temperature > 0)&&(BMT_status.temperature < 10)))
        {
            /*Set Charge current to 700mA*/
             ncp1851_set_ctrl_vbat(0x1C); //VCHG = 4.0V
             ncp1851_set_ichg(Cust_CC_700MA);
          }
//liulc1 end 
}
...
void ncp1851_set_ac_current(void)
{
    ...
    else
    {
        ...
        if(g_current_change_flag)
		 ...
		 ncp1851_set_ctrl_vbat(0x1C); //VCHG = 4.0V
		 ncp1851_set_ichg(Cust_CC_700MA); //liulc1 change
            }
...
//  ncp1851_set_int_mask(0x0); //disable all interrupt
    ...
    if((BMT_status.charger_type != STANDARD_HOST) && (BMT_status.charger_type != CHARGER_UNKNOWN))
        ncp1851_set_iweak(0x1); //weak charge current = 100mA  liulc1 change
        //ncp1851_set_iweak(0x3); //weak charge current = 300mA
    else
        ncp1851_set_iweak(0x1); //weak charge current = 100mA

    ncp1851_set_ctrl_vfet(0x1); // VFET = 3.2V  liulc1 change
...
//	        g_bat_temperature_pre = 22; // MT6320 E1 workaround
    ...
    #endif
//liulc1 add
  if ((BMT_status.temperature < 45)&&(cTransition_flag == 1))
	cTransition_flag = 0;

  if(BMT_status.temperature >= 45)
    {
        /*Set Charge current to 700mA*/
        if (cTransition_flag ==0)
        {
	    ncp1851_set_ctrl_vbat(0x1C); //VCHG = 4.0V
            ncp1851_set_ichg(Cust_CC_700MA);
            cTransition_flag = 1;
...
void mt_battery_notify_check(void)
{
    ...
    if(g_BN_TestMode == 0x0000)
    {
        ...
        /*Changed by zhangyf for Over Heat warnning*/
        if(BMT_status.temperature >= OVER_HEAT_WARNING_TEMPERATURE)
        //if(BMT_status.temperature > 20) //test
        {
            g_BatteryNotifyCode |= 0x0002;
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:ncp1851] bat_temp(%d) > 58'C\n", BMT_status.temperature);
        }
        else
        {
            g_BatteryNotifyCode &= ~(0x0002);
...
void BAT_thread_ncp1851(void)
{
    ...
    if(g_battery_thermal_throttling_flag==1)
    {
		...
		/*Changed by zhangyfh for system reset but not power off*/
...
//liulc1  fix bug 9370 Set the USB to charge only cannot read after the reboot to USB and can't charge  at 2013.3.18 update.
...
//liulc1 end
    ...
    while(1)
    {
        ...
        {
            wake_lock(&battery_suspend_lock);
//liulc1 fie bug7738 Low electric shutdown after connecting charger can't normal boot  at 2013.3.6.
