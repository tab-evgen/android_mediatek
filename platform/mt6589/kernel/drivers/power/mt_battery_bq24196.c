...
static int set_current_as_temp_bq24196(int temp_CC_value)
{
        ...
        if (Enable_BATDRV_LOG == 1) {
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:bq24196]temperature,temp = %d,old_temp = %d \r\n",temp,old_temp);
        }
...
/*end,lenovo-sw xuwei9 modify 2013-3-7, modify current by temperature*/
...
#else
/*begin,lenovo-sw xuwei9 modify 2013-3-7, modify current by temperature*/
...
/*Begin lenovo-sw xuwei9 2013-3-13,shorten charge time*/
	bq24196_set_iterm(0x1); //Termination current 128mA->256
/*End lenovo-sw xuwei9 2013-3-13,shorten charge time*/
#if defined(MTK_JEITA_STANDARD_SUPPORT)        
    if(g_temp_status == TEMP_NEG_10_TO_POS_0)
    {    
...
/*begin lenovo-sw xuwei9 2013-3-26,50-60 temperatue,keep system power path on*/
            if(charger_OVER_VOL == BMT_status.charger_protect_status )
               bq24196_set_en_hiz(0x1);
/*end lenovo-sw xuwei9 2013-3-26,50-60 temperatue,keep system power path on*/
	}
	else
	{
            bq24196_set_en_hiz(0x0);	
...
/*begin lenovo-sw xuwei9 2013-3-26,keep over vol check before temperature check*/
    if( upmu_is_chr_det() == KAL_TRUE)
    {
...
/*end lenovo-sw xuwei9 2013-3-26,keep over vol check before temperature check*/
#if defined(MTK_JEITA_STANDARD_SUPPORT)
    if (Enable_BATDRV_LOG == 1) {
...
void BAT_thread_bq24196(void)
{
    ...
    if(g_battery_thermal_throttling_flag==1)
    {
        ...
        if(BMT_status.temperature >= 60)
        {
            ...
            if( upmu_is_chr_det() == KAL_TRUE )
            {
/*begin lenovo-sw xuwei9 2013-3-18,can not power off while charger exist*/
                // can not power down due to charger exist, so need reset system
                arch_reset(0,"charger");
/*end lenovo-sw xuwei9 2013-3-18,can not power off while charger exist*/
            }
...
//    wake_lock_init(&battery_suspend_lock, WAKE_LOCK_SUSPEND, "battery wakelock");
    ...
    BMT_status.TOPOFF_charging_time = 0;
    BMT_status.POSTFULL_charging_time = 0;
/*begin lenovo-sw xuwei9 2013-3-26,50-60temperatue keep system power path on*/
    BMT_status.charger_protect_status = 0 ;
/*end lenovo-sw xuwei9 2013-3-26,50-60temperatue keep system power path on*/
    BMT_status.bat_charging_state = CHR_PRE;
