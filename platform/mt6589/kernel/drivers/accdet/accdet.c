...
unsigned int accdet_get_reg(unsigned int offset)
{
    int ret = 0;
...
void accdet_set_reg(unsigned int offset,unsigned int value, unsigned int mask)
{
    // set pmic register or analog CONTROL_IFACE_PATH
    int ret =0;
    unsigned reg_value = accdet_get_reg(offset);
    reg_value &= (~mask);
    reg_value |= (value&mask);
    ret =pwrap_write( offset, reg_value);
    reg_value = accdet_get_reg(offset);
    if((reg_value&mask)!=(value&mask))
    {
        printk("accdet_set_reg offset= 0x%x , value = 0x%x mask = 0x%x ret = %d Reg_Value = 0x%x\n",offset,value,mask,ret,reg_value);
    }
}


#define SPKR_GPIO_EN_PIN GPIO177
#define SPKL_GPIO_EN_PIN GPIO178
void accdet_turn_off_speaker(void)
{
    mt_set_gpio_dir(SPKR_GPIO_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(SPKR_GPIO_EN_PIN, GPIO_OUT_ZERO);
    mt_set_gpio_dir(SPKL_GPIO_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_dir(SPKL_GPIO_EN_PIN, GPIO_OUT_ZERO);
}

int accdet_check_pmic_audio_out_status(void)
{
	unsigned int RVal = 0;
	int ret = 0;
	ret = pwrap_read(0x700, &RVal);
	return (RVal & 0x1) ? 1 : 0;
}

#define I2S_ENABLE_ADDR		0xf2070034

void accdet_stop_pmic_audio_out(void)
{
/*
    volatile unsigned int *p = I2S_ENABLE_ADDR;
    unsigned int reg_val = *p;
  
    reg_val &= 0xfffffffe;
    *p = reg_val; 
*/
    //avoid pop noise in speaker, so shutdown pa.
    ...
    accdet_set_reg(0x700,0x0880,0xffffffff);
...
void accdet_eint_work_callback(struct work_struct *work)
{
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		cable_type = HEADSET_NO_MIC;//make a default value.
		switch_set_state((struct switch_dev *)&accdet_data, cable_type);
		#ifdef ACCDET_LOW_POWER
		...
		#ifdef ACCDET_PIN_SWAP
                if (accdet_check_pmic_audio_out_status())
                {
		    accdet_stop_pmic_audio_out();
...
void accdet_work_callback(struct work_struct *work)
{
    ...
    if(1 == eint_accdet_sync_flag) 
    {
	if(cable_type != cable && cable == HEADSET_NO_MIC && cable_type == HEADSET_MIC)
     	    switch_set_state((struct switch_dev *)&accdet_data, cable_type);
    }
    else
    {
