                                                  ...
                                                  const char *device_address)
{
    ...
    if (audio_is_output_device(device)) {
	    ...
	    //1873:[liuxm7] we won't have both wired headset and wired headphone on. remove wired headphone.
	    if ((mAvailableOutputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) && (mAvailableOutputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADPHONE))
                                             ...
                                             int session)
{
    ...
    if (outputDesc->mRefCount[stream] == 1) {
        ...
        uint32_t muteWaitMs = setOutputDevice(output, newDevice, true);
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_ENFORCED_AUDIBLE:
        ...
        if (((strategy == STRATEGY_SONIFICATION) ||
                (mForceUse[AudioSystem::FOR_SYSTEM] == AudioSystem::FORCE_SYSTEM_ENFORCED)) && (!isInCall())) {
            device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_SPEAKER;
            if (device == AUDIO_DEVICE_NONE) {
	...
	//5010:[liuxm7] 2013-2-27 Add for camera capture sound cause voice call sound from speaker.
	//This handling is quite ugly, strategy_sonification will align strategy_phone when in phone call,
	//but stategy_enforced_audible won't. I don't know the details, so workaround it here.
        if (device2 == AUDIO_DEVICE_NONE && strategy == STRATEGY_ENFORCED_AUDIBLE && isInCall()) {
                                             ...
                                             int delayMs)
{
    ...
    //here device from headphone to headset is a convention and they have the same mute strategies.
    if (!((prevDevice & AUDIO_DEVICE_OUT_WIRED_HEADPHONE) && (device & AUDIO_DEVICE_OUT_WIRED_HEADSET)))
                                            ...
                                            audio_devices_t device)
{
    ...
    if(stream == AudioSystem::BOOT)
    {
    	if (device & (AUDIO_DEVICE_OUT_BLUETOOTH_A2DP |
            ...
            volume = BOOT_ANIMATION_VOLUME*2;
	else
                                                   ...
                                                   bool force)
{
    ...
    //Lenovo_XuBo 20130308 Reduce Enforce wolume
    #if MTK_AUDIO 
    /*between mStreams[stream].mIndexMax; and mStreams[stream].mIndexMin;*/ 
    if(stream==AudioSystem::ENFORCED_AUDIBLE){// && !mStreams[stream].mCanBeMuted){
	 if(index >= mStreams[stream].mIndexMax )
	 {
	 	index -= 2;
		ALOGI("reduce volume for ENFORCE volume to = %d ",index);
	 }
    } 
    #endif 
    //Lenovo_XuBo 20130308 Reduce Enforce wolume
